...
// Update Video Likes
export async function updateVideoLikes(videoId, userId) {
  try {
    const videoDocument = await databases.getDocument(Config.databaseId, Config.videoColId, videoId);

    const likedBy = videoDocument.likedBy || [];

    if (likedBy.includes(userId)) {
      // Remove like
      const updatedLikes = likedBy.filter(id => id !== userId);
...
// Update User Likes
export async function updateUserLikes(userId, videoId) {
  try {
...
export async function getUserLikedPosts(userId) {
  try {
    ...
    if (likedVideos.length === 0) return []; // No liked videos found
